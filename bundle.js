...
/***/ function(module, exports, __webpack_require__) {
	...
	var _synth = __webpack_require__(192);
	
	var _synth2 = _interopRequireDefault(_synth);
	
	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }
	
	document.write('<canvas id="scope" width="512" height="256" style="border:1px solid #000000;">\n</canvas>\n<label for="frequency">Frequency</label>\n<input type="range" id="slider1" min="0.0" value="0.0" max="9.0" step="0.0001" oninput="frequencyUpdate(value)">\n<output for="slider" id="frequency">440</output>\n\n<label for="amplitude">Amplitude</label>\n<input type="range" id="slider1" min="0.0" value="0.0" max="1.0" step="0.0001" oninput="amplitudeUpdate(value)">\n<output for="slider" id="amplitude"></output>\n\n');
	...
	window.frequencyUpdate = function (val) {
	  // let exponent = (note - 69) / 12;
	  // let freq = Math.pow(2, exponent) * 440;
	  var freq = Math.pow(2, val) * minFreq;
	  synth.osc1.frequency.value = document.querySelector('#frequency').value = freq;
	  //  synth.osc2.frequency.value = Math.pow(2, val) * minFreq * 0.66;
	};
	
	window.amplitudeUpdate = function (val) {
	  ...
	  var alog = val; //Math.log10(10 * val); // 1 - val; // Math.log10(10 * val);
	  var glog = val; // Math.log10(10 * (1 - val));
	  synth.amplitude = val;
	  document.querySelector('#amplitude').value = synth.gain.value = val;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _scope = __webpack_require__(195);
	
	var _scope2 = _interopRequireDefault(_scope);
	
	var _osc = __webpack_require__(200);
	...
	var cheby = function cheby(k, x, cache) {
	  // if (k === 0) return 1.0;
	  // if (k === 1) return x;
	...
	module.exports = (function () {
	  function _class() {
	    ...
	    this.context = new _global.window.AudioContext();
	    this.amplitude = 0.0;
	    ...
	    this.processor.onaudioprocess = function (e) {
	      ...
	      for (var channel = 0; channel < outputBuffer.numberOfChannels; channel++) {
	        ...
	        for (var sample = 0; sample < inputBuffer.length; sample++) {
	          ...
	          var scaled = inputData[sample] * that.amplitude;
	          //            outputData[sample] = inputData[sample];
	          var cache = [];
	          outputData[sample] = inputData[sample] + cheby(11, scaled, cache) + cheby(9, scaled, cache) + cheby(7, scaled, cache) + cheby(5, scaled, cache) + cheby(3, scaled, cache) + cheby(1, scaled, cache); //+ cheby(9, scaled) +  cheby(11, scaled) + cheby(13, scaled) + cheby(15, scaled); //+ (1 - that.amplitude);
	          //            console.log(cache);
	          //            outputData[sample] = cheby(2, scaled) + cheby(4, scaled) + cheby(6, scaled) + cheby(8, scaled);
	          //            console.log(outputData[sample]);
	          //            console.log(outputData[sample]);
	          // 4 * Math.pow(scaled,3) + 2 * Math.pow(scaled, 2) - 3 * scaled + 1;
	
	          //             Math.pow(inputData[sample] * Number(that.amplitude), 3) / that.amplitude;
	          //            outputData[sample] = Math.pow(inputData[sample] * Number(that.amplitude), 3) / that.amplitude;
	          //            console.log(Number(that.amplitude));
	        }
	      }
	    };
	    ...
	    this.processor.connect(this.context.destination);
	    this.scope = new _scope2.default({ context: this.context, width: 512, height: 256 });
	    //    this.scope.draw();
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = (function () {
	  function _class() {
	    ...
	    this.canvas = _global.document.getElementById("scope");
	    this.canvasCtx = this.canvas.getContext("2d");
	  }
